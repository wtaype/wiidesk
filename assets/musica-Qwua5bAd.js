import{l as e}from"./wii-DZzdzbZC.js";import{E as t,O as n,R as r,b as i,i as a}from"./widev-DCNhLI3Q.js";import{r as o}from"./index-BBkUU4ZD.js";import{_ as s,b as c,c as l,d as u,g as d,m as f,o as p,p as m,r as h,u as g}from"./firebase-DqjnRvzz.js";import{n as _}from"./firebase-BtK1IiLA.js";var v=()=>i(`wiSmile`)||null,y=async e=>{if(!e)return[];try{let t=await p(g(d(_,`wimusicaLikes`),f(`email`,`==`,e.trim())));return t.empty?[]:t.docs.map(e=>e.data().trackId)}catch(e){return console.warn(`Error loading favorites from Firestore:`,e),[]}},b=async(e,t)=>{let n=`${e.usuario.trim().toLowerCase()}_${t.id.trim().toLowerCase()}`,r={id:n,usuario:e.usuario.trim().toLowerCase(),email:e.email,userId:e.uid,trackId:t.id,artista:t.idioma||t.cantante||`CumpleWii`,poster:t.poster||``,creado:c(),actualizado:c()};await u(s(_,`wimusicaLikes`,n),r)},x=async(e,t)=>{await h(s(_,`wimusicaLikes`,`${e.usuario.trim().toLowerCase()}_${t.trim().toLowerCase()}`))},S=null,C=[],w=[],T=!1,E=`none`,D=`todas`,O=``,k=null,A=null,j=[],M=e=>{if(isNaN(e))return`0:00`;let t=Math.floor(e/60),n=Math.floor(e%60);return`${t}:${n<10?`0`:``}${n}`},N=e=>{let t=(e,t)=>!!t.pin-+!!e.pin||(t.creado?.seconds||0)-(e.creado?.seconds||0);return[...e].sort(t)},P=()=>{let e=O.toLowerCase().trim();return C.filter(t=>{let n=`${t.titulo||``} ${t.cantante||``} ${t.idioma||``}`.toLowerCase().includes(e);return D===`todas`?n:D===`favoritos`?w.includes(t.id)&&n:t.tag?.toLowerCase().trim()===D.toLowerCase().trim()&&n})},F=(e,t,n,r={})=>{e&&(e.addEventListener(t,n,r),j.push({target:e,type:t,handler:n}))},I=()=>{let e=document.getElementById(`musPls`);if(!e)return;v();let t=P(),n=Array.from(new Set(C.map(e=>e.tag).filter(Boolean))).slice(0,8),r=document.getElementById(`catSugs`);if(r&&(r.innerHTML=n.map(e=>`<button type="button" class="cat sug_cat" style="padding:0.4vh 1.2vh; font-size:var(--fz_s3);">${e}</button>`).join(``)),!t.length){e.innerHTML=`
      <div class="mus_empty">
