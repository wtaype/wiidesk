...
let scrollPostProgHandler;
let clickPostHandler;
...
export const init = async (slug, force = false) => {
  ...
  try {
    ...
    if (!result?.data?.activo) {
      if (wimain) {
        wimain.innerHTML = `<div class="po_err dpvc"><i class="fas fa-paw"></i><h2>Historia no encontrada</h2><p>No existe o no está disponible 🐾</p><a href="/blog" class="po_back_btn"><i class="fas fa-arrow-left"></i> Ver historias</a></div>`;
      }
      return;
    }
    ...
    if (!cacheRenderizado || force) {
      if (wimain) {
        wimain.innerHTML = tplPost(p, null, null, slug, fromCache);
      }
      fade('po_fade'); showi(); setupTOC(); setupScroll();
    } else if (!fromCache) {
      document.querySelectorAll('.po_cache_badge').forEach(el => el.remove());
    }

    // Carga secundaria (Deferred Sidebar & Nav)
    const renderDeferredSidebar = (rels, ultimas) => {
      const navCont = document.getElementById('po_nav_container');
      if (navCont) navCont.innerHTML = tplNav(rels[0] || ultimas[0], rels[1] || ultimas[1]);
      
      const ultCont = document.getElementById('po_ultimas_container');
      if (ultCont) {
        ultCont.innerHTML = ultimas.length ? `<div class="po_side_card po_fade po_visible" style="--d:0s"><div class="po_side_title"><i class="fas fa-clock"></i> Últimas historias</div><div class="po_relacionados" data-showi="100">${ultimas.map(tplRel).join('')}</div></div>` : '';
      }
      
      const relsCont = document.getElementById('po_rels_container');
      if (relsCont) {
        relsCont.innerHTML = rels.length ? `<div class="po_side_card po_fade po_visible" style="--d:0s"><div class="po_side_title"><i class="fas fa-heart"></i> Más historias de ${p.categoria}</div><div class="po_relacionados" data-showi="100">${rels.map(tplRel).join('')}</div></div>` : '';
      }
      showi();
    };
    ...
    if (window.DISQUS) {
      window.DISQUS.reset({reload:true,config:dCfg});
    } else {
      window.disqus_config=dCfg;
      wiSmart({
        disqus: [
          async () => {
...
// ── HELPERS ───────────────────────────────────────────────────
function setupTOC() {
  const tocBox = document.querySelector('.po_toc_box');
  if (tocBox) return;
  const contenido = document.querySelector('.po_contenido');
  if (!contenido) return;
  const headings = contenido.querySelectorAll('h2,h3');
  ...
  headings.forEach((el, i) => {
    ...
    el.style.scrollMarginTop = '7vh';
    html += `<li style="margin-left:${el.tagName.toLowerCase() === 'h3' ? '1.5vh' : '0'}"><a href="#${id}" class="po_toc_link">${el.textContent}</a></li>`;
  });
  html += '</ul></div>';
  contenido.insertAdjacentHTML('beforebegin', html);
}

function setupScroll() {
  if (scrollPostProgHandler) {
    window.removeEventListener('scroll', scrollPostProgHandler);
  }
  scrollPostProgHandler = () => {
    const poProgress = document.getElementById('po_progress');
    if (poProgress) {
      const scrollTop = window.scrollY || document.documentElement.scrollTop;
      const docHeight = document.documentElement.scrollHeight;
      const winHeight = window.innerHeight;
      const pct = (scrollTop / Math.max(docHeight - winHeight, 1)) * 100;
      poProgress.style.width = pct + '%';
    }
  };
  window.addEventListener('scroll', scrollPostProgHandler);
}

// ── EVENTOS GLOBALES ──────────────────────────────────────────
clickPostHandler = async (e) => {
  // 1. click on .po_copy, .po_copy2
  const copyBtn = e.target.closest('.po_copy, .po_copy2');
  if (copyBtn) {
    wicopy(location.href, '.po_copy', '¡Enlace copiado! 🔗');
    return;
  }

  // 2. click on .po_rel_card
  const relCard = e.target.closest('.po_rel_card');
  if (relCard) {
    e.preventDefault();
    const href = relCard.getAttribute('href');
    if (href) {
  ...
  // 3. click on .po_like_sync
  const likeSync = e.target.closest('.po_like_sync');
  if (likeSync) {
    const s = likeSync.getAttribute('data-slug');
    if (localStorage.getItem('wi_like_' + s)) return;
    localStorage.setItem('wi_like_' + s, '1');
    document.querySelectorAll('.po_like_sync').forEach(el => el.classList.add('active'));
    
    const countTextEl = document.querySelector('.po_likes_count_text');
    const firstCountVal = countTextEl ? parseInt(countTextEl.textContent, 10) || 0 : 0;
    document.querySelectorAll('.po_likes_count_text').forEach(el => {
      el.textContent = (firstCountVal + 1).toString();
    });
  ...
  // 4. click on .po_yt_btn
  const ytBtn = e.target.closest('.po_yt_btn');
  if (ytBtn) {
    const id = ytBtn.getAttribute('data-yt');
    let ytModal = document.getElementById('wi_yt_modal');
    if (!ytModal) {
      const div = document.createElement('div');
      div.id = 'wi_yt_modal';
      div.className = 'wiModal';
      div.innerHTML = `
      ...
      ytModal.addEventListener('click', function(e) {
        if (e.target === this) {
          const closeBtn = document.querySelector('.wi_yt_close');
          if (closeBtn) closeBtn.click();
        }
      });
    }
    
    const ytPlayer = document.getElementById('wi_yt_player');
    if (ytPlayer) {
  ...
  // 5. click on .wi_yt_close
  const ytClose = e.target.closest('.wi_yt_close');
  if (ytClose) {
  ...
  // 6. click on #po_refresh
  const refreshBtn = e.target.closest('#po_refresh');
  if (refreshBtn) {
    const originalHtml = refreshBtn.innerHTML;
    refreshBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i>';
    refreshBtn.disabled = true;
    const slug = refreshBtn.getAttribute('data-slug');
    const cat = refreshBtn.getAttribute('data-cat');
    clearPostCache(slug);
    clearRelCache(cat);
...
document.addEventListener('click', clickPostHandler);
...
export const cleanup = () => {
  ...
  if (clickPostHandler) {
    document.removeEventListener('click', clickPostHandler);
  }
};
