...
const _syncSidebarSelection = () => {
  ...
  document.querySelectorAll('.chat_sidebar_item').forEach(el => {
    const fullName = (el.getAttribute('data-nombre') || '').trim();
    if (!fullName) return;
...
const _renderMessages = (preventScroll = false) => {
  ...
  // Animate-in stagger
  msgs.querySelectorAll('.chat_msg_in').forEach((el, i) => {
...
const _loadMensajes = async (forceReload = false) => {
  ...
  if (forceReload && chatRefresh) {
    chatRefresh.classList.add('chat_spinning');
  }

  const chatHeader = document.getElementById('chatHeader');
  if (chatHeader) chatHeader.classList.add('smw_loading');
  ...
  try {
    ...
    cargando = false;
    if (chatHeader) chatHeader.classList.remove('smw_loading');
    if (chatRefresh) chatRefresh.classList.remove('chat_spinning');
  }
};
...
const _loadAnteriores = async () => {
  ...
  cargando = true;
  const btn = document.getElementById('chatPaginateBtn');
  if (btn) {
    btn.disabled = true;
    btn.innerHTML = '<i class="fas fa-circle-notch fa-spin"></i> Cargando anteriores...';
  }
...
const _enviarMensaje = async () => {
  ...
  ta.value = '';
  // Dispatch input event to trigger resize / sync
...
const _updateSendUI = () => {
  ...
  const blk = document.getElementById('chatBlockedMsg');

  if (puedeEnviar) {
    if (ta) {
      ta.disabled = false;
      ta.setAttribute('placeholder', 'Escribe un mensaje…');
    }
    if (btn) {
      btn.disabled = false;
      btn.classList.remove('chat_send_disabled');
    }
    if (blk) blk.style.display = 'none';
  } else {
    if (ta) {
      ta.disabled = true;
      ta.setAttribute('placeholder', 'No puedes enviar mensajes (cuenta inactiva o sin permisos)');
    }
    if (btn) {
      btn.disabled = true;
      btn.classList.add('chat_send_disabled');
    }
...
export const init = async () => {
  cleanup(); // Reset and clean up existing listeners
  ...
  onDoc('input', '#chatTextarea', function () {
    _autoResize(this);
    _syncSidebarSelection();
  });

  // Send on Enter (Shift+Enter = newline)
  onDoc('keydown', '#chatTextarea', function (e) {
  ...
  // Send button click
  onDoc('click', '#chatSendBtn', () => _enviarMensaje());

  // Refresh button
  onDoc('click', '#chatRefresh', async function () {
  ...
  onDoc('click', '#chatPaginateBtn', () => _loadAnteriores());

  // Delete message click: open custom modal
  onDoc('click', '.chat_msg_delete_btn', function () {
    const id = this.getAttribute('data-id');
    if (!id) return;
    msgIdAEliminar = id;
    const modal = document.getElementById('chatEliminarModal');
    if (modal) modal.classList.add('show');
  });

  // Cancel deletion or backdrop click
  onDoc('click', '#chatCancelDeleteBtn, #chatEliminarModal', function (e) {
    if (e.target.id === 'chatCancelDeleteBtn' || e.target.id === 'chatEliminarModal') {
      const modal = document.getElementById('chatEliminarModal');
  ...
  onDoc('click', '#chatConfirmDeleteBtn', async function () {
    ...
    try {
      ...
      const bubbleWrap = document.querySelector(`.chat_bubble_wrap[data-id="${id}"]`);
      if (bubbleWrap) {
        bubbleWrap.style.transition = 'opacity 0.3s';
  ...
  onDoc('input', '#chatSearchInput', function () {
    filterText = this.value;
    _renderSidebar();
  });

  // Click on collaborator to mention them (Smart Toggle Selection V3)
  onDoc('click', '.chat_sidebar_item', function (e) {
    if (!puedeEnviar) return;

    const fullName = (this.getAttribute('data-nombre') || '').trim();
    const ta = document.getElementById('chatTextarea');
    if (!ta) return;
    ...
    if (isActive) {
      ...
      ta.value = currentText + prefix + mention + ' ';
    }
    ...
    if (container) container.classList.remove('chat_sidebar_active');
  });

  // Toggle sidebar on mobile
  onDoc('click', '#chatSidebarToggle', function () {
    const container = document.getElementById('chatContainer');
    if (container) container.classList.add('chat_sidebar_active');
  });

  onDoc('click', '#chatSidebarClose', function () {
    const container = document.getElementById('chatContainer');
    if (container) container.classList.remove('chat_sidebar_active');
  });

  // Retry button
  onDoc('click', '#chatRetry', () => {
    const msgs = document.getElementById('chatMessages');
    if (msgs) msgs.innerHTML = _skeletons();
    const list = document.getElementById('chatSidebarList');
    if (list) list.innerHTML = _sidebarSkeletons();
    _loadMensajes(true);
    _loadColaboradores();
  });

  // Scroll to bottom on focus (mobile)
  onDoc('focus', '#chatTextarea', function () {
